...
for lambda_value in lambda_values:
    ...
    system.setDefaultPeriodicBoxVectors(
        Vec3(box_edge, 0, 0), Vec3(0, box_edge, 0), Vec3(0, 0, box_edge)
    )
...
positions = Quantity(
    numpy.random.uniform(high=box_edge / angstroms, size=[nparticles, 3]), angstrom
)
...
u_kln = numpy.zeros(
    [nlambda, nlambda, nprod_iterations], numpy.float64
)  # u_kln[k,l,n] is reduced potential of snapshot n from simulation k at alchemical state l
for lambda_index, lambda_value in enumerate(lambda_values):
    ...
    for iteration in range(nprod_iterations):
        print(
            ...
            % (lambda_index + 1, nlambda, iteration + 1, nprod_iterations)
        )
    ...
    print(
        ...
        % (lambda_index + 1, nlambda)
    )
    beta = 1.0 / (
        BOLTZMANN_CONSTANT_kB * AVOGADRO_CONSTANT_NA * temperature
    )  # inverse temperature
...
try:
    ...
    raise ImportError(
        "pymbar [https://pymbar.readthedocs.io/] must be installed to complete analysis of free energies."
    )
...
u_kln_subsampled = numpy.zeros(
    [K, K, nprod_iterations], numpy.float64
)  # subsampled data
for k in range(K):
...
# Report result.
# =============================================================================
print(Deltaf_ij[0, K - 1] / beta)
print(
    "Free energy of inserting argon particle: %.3f +- %.3f kT"
    % (Deltaf_ij[0, K - 1] / beta),
    (dDeltaf_ij[0, K - 1] / beta),
)
